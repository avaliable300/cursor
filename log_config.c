...
static void log_default_config(LogConfig* config) {
    ...
    snprintf(config->log_file, sizeof(config->log_file), "./log_default.log");
    config->use_color = false;
    config->use_lock = false;
    config->max_line = 1000;
    config->max_memory = 3.0f;
...
static void parse_log_config_line(char* line, LogConfig* config) {
    ...
    if (strcmp(key, "level") == 0) {
        if (strcmp(value, "trace") == 0) config->log_level = 0;       // LOG_TRACE
        else if (strcmp(value, "debug") == 0) config->log_level = 1;  // LOG_DEBUG
        else if (strcmp(value, "info") == 0)  config->log_level = 2;  // LOG_INFO
        else if (strcmp(value, "warn") == 0)  config->log_level = 3;  // LOG_WARN
        else if (strcmp(value, "error") == 0) config->log_level = 4;  // LOG_ERROR
        else if (strcmp(value, "fatal") == 0) config->log_level = 5;  // LOG_FATAL
        else if (strcmp(value, "off") == 0)   config->log_level = -1; // OFF
